...
import { AlertTriangle, Utensils, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
const NudgeStack = () => {
  ...
  // Auto-rotate through nudges, prioritizing unread/actionable
  useEffect(() => {
  ...
  const accentColor = current.type === 'action'
    ? 'text-accent'
    : current.type === 'update'
    ? 'text-primary'
  ...
  return (
    <div className="w-full">
      {/* Permanent notification strip */}
      <div className="bg-card/60 backdrop-blur-sm border-b border-border">
        {/* Ticker header */}
        <div className="flex items-center justify-between px-4 pt-2 pb-1">
          <div className="flex items-center gap-2">
            <div className={`w-1.5 h-1.5 ${unreadCount > 0 ? 'bg-accent' : 'bg-muted-foreground/30'}`} />
        ...
        {/* Current nudge — elegant single display */}
        <div className="relative px-4 pb-3 min-h-[56px]">
          <AnimatePresence mode="wait" initial={false} custom={direction}>
            ...
            >
              <Icon size={13} className={`mt-0.5 shrink-0 ${accentColor}`} />
              <div className="flex-1 min-w-0">
                ...
                {current.actionLabel && (
                  <button className="mt-2 px-3 py-1.5 bg-primary text-primary-foreground font-sans text-[9px] uppercase tracking-sovereign border-none cursor-pointer">
                    {current.actionLabel}
                  </button>
                )}
              </div>
          ...
          {nudges.length > 1 && (
            <>
              <button
                onClick={() => goTo(-1)}
                className="absolute left-0 top-1/2 -translate-y-1/2 w-6 h-10 flex items-center justify-center bg-transparent border-none cursor-pointer opacity-30 hover:opacity-60 transition-opacity"
                aria-label="Previous"
              >
              ...
              <button
                onClick={() => goTo(1)}
                className="absolute right-0 top-1/2 -translate-y-1/2 w-6 h-10 flex items-center justify-center bg-transparent border-none cursor-pointer opacity-30 hover:opacity-60 transition-opacity"
                aria-label="Next"
              >
